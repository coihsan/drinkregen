...
// Kita passing objek request dari Astro ke fungsi ini
export async function assertCanAccessActivityLogs(request: Request) {
  const session = await auth.api.getSession({
    headers: request.headers, // Menggunakan headers dari standar Request web API
  });
